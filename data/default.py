...
cmd_config = (
    conquest.createCommand(name="config", description="Retrieve and update agent settings.", example="config --sleep 10 --jitter 15 --sleepmask ekko",
                           message="Tasked agent to retrieve and update agent settings.", mitre=["T1029"], category="CORE")
...
cmd_exit = (
    conquest.createCommand(name="exit", description="Exit the agent.", example="exit process", message="Tasked agent to exit.", category="API")
...
cmd_selfdestruct = (
    conquest.createCommand(name="self-destruct", description="Exit the agent and delete the executable from disk.", example="self-destruct", 
                           message="Tasked agent to self-destruct.", mitre=["T1070.004"], category="API")
            .registerToGroup("core")
)

cmd_link = (
    conquest.createCommand(name="link", description="Create a link to a SMB agent.", example="link DC01 msagent_1234", 
                           message="Tasked agent to link to SMB agent.", mitre=["T1021.002", "T1090.001"], category="CORE")
...
cmd_unlink = (
    conquest.createCommand(name="unlink", description="Remove a link to a SMB agent.", example="unlink C804A284", message="Tasked agent to unlink SMB agent.", category="CORE")
            .addArgString("agent", "ID of the agent to unlink.", True)
            .registerToGroup("core")
)

cmd_links = (
    conquest.createCommand(name="links", description="List linked agents.", example="links", message="Tasked agent to list linked agents.", category="CORE")
            .registerToGroup("core")
)

cmd_jobs = (
    conquest.createCommand(name="jobs", description="List running jobs.", example="jobs", message="Tasked agent to list jobs.", category="CORE")
            .registerToGroup("core")
)

cmd_cancel = ( 
    conquest.createCommand(name="cancel", description="Cancel a running job.", example="cancel DEADBEEF", message="Tasked agent cancel a job.", category="CORE")
...
cmd_shell = (
    conquest.createCommand(name="shell", description="Execute a shell command and retrieve the output.", example="shell whoami /all", 
                           message="Tasked agent to execute shell command and retrieve the output.", mitre=["T1059.003"], category="API")
...
cmd_bof = (
    conquest.createCommand(name="bof", description="Execute an object file in memory and retrieve the output.", example="bof /path/to/whoami.x64.o", 
                           message="Tasked agent to execute an object-file in memory and retrieve the output.", mitre=["T1055", "T1620"], category="API")
            ...
            .addFlagString("--entry", "function", "Entry point of the object file (default: go).", False, "go")
            .registerToGroup("execution")
...
cmd_dotnet = (
    conquest.createCommand(name="dotnet", description="Execute a .NET assembly in memory and retrieve the output.", example="dotnet /path/to/SharpHound.exe -c all -d domain.local", 
                           message="Tasked agent to execute a .NET assembly in memory and retrieve the output.", mitre=["T1055", "T1620"], category="API")
...
cmd_dll = (
    conquest.createCommand(name="dll", description="Execute a DLL asynchronously in memory.", example="dll /path/to/async-bof.dll Run <packed-args>", 
                           message="Tasked agent to execute a DLL in memory.", mitre=["T1620"], category="API")
...
# Post-exploitation
cmd_download = (
    conquest.createCommand(name="download", description="Download a file (async).", example="download C:\\Users\\john\\Documents\\Database.kdbx", 
                           message="Tasked agent to download file.", mitre=["T1005", "T1041"], category="API")
...
cmd_upload = (
    conquest.createCommand(name="upload", description="Upload a file.", example="upload /path/to/payload.exe", 
                           message="Tasked agent to upload file.", mitre=["T1544"], category="API")
...
cmd_ps = (
    conquest.createCommand(name="ps", description="Display running processes.", example="ps", 
                           message="Tasked agent to display running processes.", mitre=["T1424"], category="API")
...
cmd_pwd = (
    conquest.createCommand(name="pwd", description="Retrieve current working directory.", example="pwd",
                           message="Tasked agent to retrieve current working directory.", mitre=["T1083"], category="API")
...
cmd_cd = (
    conquest.createCommand(name="cd", description="Change current working directory.", example="cd C:\\Windows\\Tasks", 
                           message="Tasked agent to change working directory.", mitre=["T1083"], category="API")
...
cmd_ls = (
    conquest.createCommand(name="ls", description="List files and directories.", example="ls C:\\Users\\Administrator\\Desktop", 
                           message="Tasked agent to list files and directories.", mitre=["T1083"], category="API")
...
cmd_screenshot = (
    conquest.createCommand(name="screenshot", description="Take and retrieve a screenshot of the target desktop.", example="screenshot", 
                           message="Tasked agent to take a screenshot of the target desktop.", mitre=["T1113"], category="API")
            .registerToGroup("situational awareness")
...
cmd_maketoken = (
    conquest.createCommand(name="make-token", description="Create an access token from username and password.", example="make-token LAB\\john Password123!", 
                           message="Tasked agent to create an access token from username and password.", mitre=["T1134.003"], category="API")
...
cmd_stealtoken = (
    conquest.createCommand(name="steal-token", description="Steal the primary access token of a remote process.", example="steal-token 1234", 
                           message="Tasked agent to steal an access token.", mitre=["T1134.001"], category="API")
...
cmd_usetoken = (
    conquest.createCommand(name="use-token", description="Use and impersonate access token from the vault.", example="use-token 1",
                           message="Tasked agent to use a token from the vault.", mitre=["T1134"], category="API")
...
cmd_rev2self = (
    conquest.createCommand(name="rev2self", description="Revert to original access token.", example="rev2self", 
                           message="Tasked agent to revert to original access token.", mitre=[], category="API")
            .setOutputHandler(lambda agentId, output: (
...
cmd_tokenvault = ( 
    conquest.createCommand(name="token-vault", description="List access tokens stored in the vault.", example="token-vault",
                           message="Tasked agent to list token vault.", mitre=[], category="API")
            .registerToGroup("user impersonation")
            .registerToModule("token")
)
...
cmd_tokeninfo = (
    conquest.createCommand(name="token-info", description="Retrieve information about the current access token.", example="token-info", 
                           message="Tasked agent to retrieve information about the current access token.", category="API")
            .registerToGroup("user impersonation")
            .registerToModule("token")
)
...
cmd_enablepriv = (
    conquest.createCommand(name="enable-privilege", description="Enable a token privilege.", example="enable-privilege SeImpersonatePrivilege", 
                           message="Tasked agent to enable a token privilege.", mitre=["T1134"], category="API")
...
cmd_disablepriv = (
    conquest.createCommand(name="disable-privilege", description="Disable a token privilege.", example="disable-privilege SeImpersonatePrivilege", 
                           message="Tasked agent to disable a token privilege.", mitre=["T1134"], category="API")
