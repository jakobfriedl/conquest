...
proc drawTable(component: SessionsComponent) =
...
proc updateGraph(component: SessionsComponent) =
    var layoutChanged = false

    if not component.graph.hasNode(SERVER_NODE_ID):
        component.graph.addNode(SERVER_NODE_ID, "Team Server")
        layoutChanged = true

    for agentId, agent in component.agents:
        let label = agent.agentId & "\t" & $agent.pid & "/" & agent.process & "\t" & agent.username & "\t" & agent.hostname
        if component.graph.hasNode(agentId):
            component.graph.updateNode(agentId, label, agent.elevated)
        else:
            component.graph.addNode(agentId, label, elevated = agent.elevated)
            layoutChanged = true

    var toRemove: seq[string]
    for nodeId in component.graph.nodes.keys:
        if nodeId != SERVER_NODE_ID and not component.agents.hasKey(nodeId):
            toRemove.add(nodeId)
    for id in toRemove:
        component.graph.removeNode(id)
        layoutChanged = true

    component.graph.clearEdges()
    for agentId in component.agents.keys:
        component.graph.addEdge(agentId, SERVER_NODE_ID, EDGE_HTTP)

    if layoutChanged:
        component.graph.applyLayout()
...
proc draw*(component: SessionsComponent) =
    ...
    if component.showGraph[]:
        ...
        let selection = component.graph.draw()
