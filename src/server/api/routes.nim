...
proc httpGet*(request: Request, profile: Profile) =
    {.cast(gcsafe).}:

        # Check heartbeat metadata placement
        var heartbeatString: string

        case profile.getString("http-get.agent.heartbeat.placement.type"):
        of "header":
            let heartbeatHeader = profile.getString("http-get.agent.heartbeat.placement.name")
        ...
        of "query":
            let param = profile.getString("http-get.agent.heartbeat.placement.name")
        ...
        let heartbeat = profile.reverseDataTransformation("http-get.agent.heartbeat", heartbeatString)
        ...
        try:
            ...
            let payload = profile.applyDataTransformation("http-get.server.output", responseBytes)

            # Add headers, as defined in the team server profile
            var headers: HttpHeaders
            for header in profile.getTableKeys("http-get.server.headers"):
...
proc httpPost*(request: Request, profile: Profile) =
    {.cast(gcsafe).}:
        ...
        try:
            ...
            case profile.getString("http-post.agent.output.placement.type"):
            of "header":
                let dataHeader = profile.getString("http-post.agent.output.placement.name")
            ...
            of "query":
                let param = profile.getString("http-post.agent.output.placement.name")
            ...
            let data = profile.reverseDataTransformation("http-post.agent.output", dataString)

            # Add response headers, as defined in team server profile
            var headers: HttpHeaders
            for header in profile.getTableKeys("http-post.server.headers"):
                headers.add((header.key, header.value.getStringValue()))
            ...
            request.respond(200, body = profile.getString("http-post.server.output.body"))
